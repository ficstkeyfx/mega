...
public class RedEyeDevil extends ParticularObject {
    ...
    public RedEyeDevil(float x, float y, GameWorldState gameWorld) {
        ...
        forwardAnim.flipAllImage(); // Đảo ngược animation redeye 
        startTimeToShoot = 0; // Thời điểm bắt đầu bắn
        setDamage(10); // damage khi megaman đụng phải 
        setTimeForNoBehurt(300000000);
        shooting = CacheDataLoader.getInstance().getSound("redeyeshooting");
    }
    // Tấn công bằng cách tạo ra đạn 
    @Override
    public void attack() {
        ...
        getGameWorld().bulletManager.addObject(bullet); // Lấy đạn vào để game update
    ...
    public void draw(Graphics2D g2) {
        if(!isObjectOutOfCameraView()){
            if(getState() == NOBEHURT && (System.nanoTime()/10000000)%2!=1){ // Nháy khi bị bắn
                // plash...
            }else{
                // Hướng trái thì vẽ back phải thì forward
