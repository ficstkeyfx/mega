...
public class GameWorldState extends State {
    ...
    public static final int finalBossX = 3600; //Vị trí boss cuối
    // Các game state
    ...
    public int storyTutorial = 0; //index để lấy texts
    ...
    public GameWorldState(GamePanel gamePanel){
        ...
        particularObjectManager = new ParticularObjectManager(this); // Tạo mới
        particularObjectManager.addObject(megaMan); // Add megaman vào particular object manager
        
        initEnemies(); // Tạo quái và thêm vào particular object manager

        bgMusic = CacheDataLoader.getInstance().getSound("bgmusic");
        
    }
    // Tạo quái và thêm vào particular object manager
            ...
            case MEETFINALBOSS:
                ...
                if(storyTutorial == 0){
                    ...
                    if(openIntroGameY < 450 && camera.getPosX() >= finalBossX && megaMan.getPosX() >= finalBossX + 150){ 
                        camera.lock(); // lock camera không di chuyển theo megaman nữa
                        storyTutorial++;
                        megaMan.stopRun();
                        // Đóng cổng cả physmap và backgrmap
    ...
    // Viết lại draw string có thể xuống dòng được
